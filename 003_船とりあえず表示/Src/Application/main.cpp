...
{
	auto _transMat = Math::Matrix::CreateTranslation(0, 20, -70);
...
void Application::Draw()
{
	...
	{
		static float deg = NULL;
		deg += 0.1f;
		if (deg > 360)deg -= 360;
		auto _rotMat = Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(deg));
		KdShaderManager::Instance().m_StandardShader.DrawModel(m_model,_rotMat);
	}
...
bool Application::Init(int w, int h)
{
	...
	// とりあえず船
	m_model.Load("Asset/Models/ship/GalleonOGA.gltf");
