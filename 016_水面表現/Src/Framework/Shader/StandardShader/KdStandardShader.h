...
public:
	...
	struct cbObject
	{
		...
		Math::Matrix mR; // 90度回転用

		Math::Vector2 WaterUVOffset = Math::Vector2::Zero; // オフセット:補正値
		float dummy[2] = { 0,0 };
	};
	...
	// 水面表現４セッターを作成
	// Textureを転送する関数
	void SetNormalTexture(KdTexture& _normalTex)
	{
		KdDirect3D::Instance().WorkDevContext()->PSSetShaderResources(
			9,	// スロット番号（4～9）
			1,	// 個数
			_normalTex.WorkSRViewAddress()	// テクスチャ
		);
	}
	// 有効化
	void SetWaterEnable(const bool _enable)
	{
