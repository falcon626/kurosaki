...
public :
	...
	// レンダーターゲット切り替え対応
	void ChangeRenderTager();
	void UndoRenderTarget();
	const std::shared_ptr<KdTexture>& GetRenderTargetTex() const
	{
		return m_rtPack.m_RTTexture;
	}

	const std::shared_ptr<KdTexture>& GetRenderTargetZTex() const
	{
		return m_rtPack.m_ZBuffer;
	}
