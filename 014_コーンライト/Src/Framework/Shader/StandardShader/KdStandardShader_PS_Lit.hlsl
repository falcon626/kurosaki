...
// コーンライトのライティング処理
{
	...
	// 光源の影響範囲に入っているか確認
		if (g_ConeLight.range > len)
		{
		// 距離内の場合
		// 光源に向いたベクトルを正規化（方向ベクトルに変更）
			toDir = normalize(toDir);

		// 光源に向いたベクトルと光源の逆ベクトルとの角度を出す
			float rad = acos(dot(toDir, -g_ConeLight.dir)); // saturateは不要
		...
		// 光源の角度内か確認
			...
			// コーンライト内に確定
			// 光源の内側から外側への減衰処理
				float angleIn = g_ConeLight.angle * 0.8; // 内側の減衰開始角度
				float side = saturate(1 - (rad - angleIn) / (g_ConeLight.angle - angleIn));

			// 距離に基づく減衰
				float atte = saturate(1 - len / g_ConeLight.range) * side;

			// 法線と光源に向いたベクトルの間の角度による拡散光の計算
